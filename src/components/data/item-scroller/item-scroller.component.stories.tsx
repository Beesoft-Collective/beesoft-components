...
import { data1, data2, data3, data4 } from './item-scroller-test-common.ts';
...
const TestComponent = () => {
  ...
  const onRequestData = (page: number) => {
    if (page === 1) {
      setScrollerData(data1);
    } else if (page === 2) {
      setScrollerData([...data1, ...data2]);
    } else if (page === 3) {
      setScrollerData([...data1, ...data2, ...data3]);
    } else if (page === 4) {
      setScrollerData([...data1, ...data2, ...data3, ...data4]);
    }
  };
